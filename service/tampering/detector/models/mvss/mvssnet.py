"""MVSS-Net model (Chen et al., ICCV 2021) — vendored from dong03/MVSS-Net.

Changes from upstream:
  * `resnet()` honours `pretrained=False` (upstream unconditionally downloaded
    ResNet-50 weights via `torch.utils.model_zoo`; the full checkpoint we load
    overrides those anyway, so skipping avoids a network dependency).
  * Informational prints removed — they pollute library output.
"""
import numpy as np
import torch
...
class BayarConv2d(nn.Module):
    ...
    def forward(self, x):
        return F.conv2d(x, self.bayarConstraint(), stride=self.stride, padding=self.padding)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, n_input=3):
        ...
        rates = [1, 2, 4]
        self.layer4 = self._make_deeplabv3_layer(
            block, 512, layers[3], rates=rates, stride=1,
        )
    ...
    def forward(self, x):
        ...
        x = self.layer1(x); x = self.layer2(x); x = self.layer2(x)
...
def resnet(
    pretrained=False, layers=(3, 4, 6, 3), backbone="resnet50", n_input=3, **kwargs,
):
    """ResNet-50 for MVSS-Net backbone.

    Upstream always fetched ImageNet weights; here we skip when the caller
    plans to load a full MVSS-Net checkpoint (which overrides these weights
    anyway) to avoid a network dependency on every model construction.
    """
    model = ResNet(Bottleneck, list(layers), n_input=n_input, **kwargs)
    if pretrained:
        pretrain_dict = model_zoo.load_url(_MODEL_URLS[backbone])
        try:
            model.load_state_dict(pretrain_dict, strict=False)
        except Exception:
            clean = {k: v for k, v in pretrain_dict.items() if "conv1" not in k}
...
class ResNet50(nn.Module):
    def __init__(self, pretrained=False, n_input=3):
        super().__init__()
        self.model = resnet(
            n_input=n_input, pretrained=pretrained, layers=[3, 4, 6, 3],
            backbone="resnet50",
        )
        self.relu = self.model.relu
        layers_cfg = [4, 5, 6, 7]
        self.blocks = [list(self.model.children())[n] for n in layers_cfg]
...
class ERB(nn.Module):
    ...
    def forward(self, x, relu=True):
        x = self.conv1(x)
        res = self.conv2(x); res = self.bn(res); res = self.relu(res)
        res = self.conv3(res)
...
class MVSSNet(ResNet50):
    def __init__(self, nclass, aux=False, sobel=False, constrain=False, n_input=3, **kwargs):
        pretrained_base = kwargs.pop("pretrained_base", False)
        super().__init__(pretrained=pretrained_base, n_input=n_input)
    ...
    def forward(self, x):
        ...
        if self.sobel:
            ...
            res1 = self.erb_trans_1(
                res1 + self.upsample(self.erb_db_2(run_sobel(self.sobel_x2, self.sobel_y2, c2))),
            )
            res1 = self.erb_trans_2(
                res1 + self.upsample_4(self.erb_db_3(run_sobel(self.sobel_x3, self.sobel_y3, c3))),
            )
            res1 = self.erb_trans_3(
                res1 + self.upsample_4(self.erb_db_4(run_sobel(self.sobel_x4, self.sobel_y4, c4))),
                relu=False,
            )
        else:
        ...
        if self.constrain:
            ...
            c4 = torch.cat([c4, constrain_features[-1]], dim=1)

        head_out = self.head(c4)
        x0 = F.interpolate(head_out[0], size, mode="bilinear", align_corners=True)
        outputs = [x0]
        if self.aux:
            outputs.append(F.interpolate(head_out[1], size, mode="bilinear", align_corners=True))
            outputs.append(F.interpolate(head_out[2], size, mode="bilinear", align_corners=True))
...
class _ChannelAttentionModule(nn.Module):
    ...
    def forward(self, x):
        b, _, h, w = x.size()
        feat_a = x.view(b, -1, h * w)
        feat_at = x.view(b, -1, h * w).permute(0, 2, 1)
        attention = torch.bmm(feat_a, feat_at)
...
class _DAHead(nn.Module):
    def __init__(self, in_channels, nclass, aux=True, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.aux = aux
        inter = in_channels // 4
        nk = {} if norm_kwargs is None else norm_kwargs
        self.conv_p1 = nn.Sequential(
            nn.Conv2d(in_channels, inter, 3, padding=1, bias=False),
            norm_layer(inter, **nk), nn.ReLU(True),
        )
        self.conv_c1 = nn.Sequential(
            nn.Conv2d(in_channels, inter, 3, padding=1, bias=False),
            norm_layer(inter, **nk), nn.ReLU(True),
        )
        self.pam = _PositionAttentionModule(inter, **kwargs)
        ...
        self.out = nn.Sequential(nn.Dropout(0.1), nn.Conv2d(inter, nclass, 1))
        if aux:
            self.conv_p3 = nn.Sequential(nn.Dropout(0.1), nn.Conv2d(inter, nclass, 1))
            self.conv_c3 = nn.Sequential(nn.Dropout(0.1), nn.Conv2d(inter, nclass, 1))

    def forward(self, x):
        feat_p = self.conv_p1(x); feat_p = self.pam(feat_p); feat_p = self.conv_p2(feat_p)
        feat_c = self.conv_c1(x); feat_c = self.cam(feat_c); feat_c = self.conv_c2(feat_c)
...
def get_mvss(
    backbone="resnet50", pretrained_base=False, nclass=1, sobel=True,
    n_input=3, constrain=True, **kwargs,
):
    return MVSSNet(
        nclass, backbone=backbone, pretrained_base=pretrained_base,
        sobel=sobel, n_input=n_input, constrain=constrain, **kwargs,
    )
