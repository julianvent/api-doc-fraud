...
class DecoderBlock(nn.Module):
    def __init__(self,cin,cadd,cout,):
        ...
        self.cin = (cin + cadd)
        self.cout = cout
        self.conv1 = md.Conv2dReLU(self.cin,self.cout,kernel_size=3,padding=1,use_norm="batchnorm")
        self.conv2 = md.Conv2dReLU(self.cout,self.cout,kernel_size=3,padding=1,use_norm="batchnorm")
...
class DTD(SegmentationModel):
    def __init__(self, encoder_name="resnet18", decoder_channels=(384, 192, 96, 64),
                 classes=1, vph_path="vph_imagenet.pt", swin_path="swin_imagenet.pt",
                 device="cpu"):
        super().__init__()
...
class seg_dtd(nn.Module):
    def __init__(self, model_name="resnet18", n_class=1,
                 vph_path="vph_imagenet.pt", swin_path="swin_imagenet.pt",
                 device="cpu"):
        super().__init__()
        self.model = DTD(
            encoder_name=model_name, classes=n_class,
            vph_path=vph_path, swin_path=swin_path, device=device,
        )

    def forward(self, x, dct, qt):
        return self.model(x, dct, qt)
